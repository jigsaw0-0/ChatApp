...
func restartChat(chatRoomId: String, memberIds: [String]) {
    ...
    XMPPUserListener.shared.downloadUsersFromFirebase(withIds: memberIds) { (users) in
...
func createRecentItems(chatRoomId: String, users: [User]) {
    ...
    FirebaseReference(.Recent).whereField(kCHATROOMID, isEqualTo: chatRoomId).getDocuments { (snapshot, error) in
        ...
        for userId in memberIdsToCreateRecent {
            ...
            let recentObject = RecentChat.init(id: UUID().uuidString, chatRoomId: chatRoomId, senderId: senderUser.id, senderName: senderUser.username, receiverId: receiverUser.id, receiverName: receiverUser.username, date: Date(), memberIds: [senderUser.id, receiverUser.id], lastMessage: "", unreadCounter: 0, avatarLink: receiverUser.avatarLink)
            
            XMPPRecentListener.shared.saveRecent(recentObject)
        }
...
func chatRoomIdFrom(user1Id: String, user2Id: String) -> String {
    ...
    if let currentUserXMPP = User.currentUserXMPP {
        return currentUserXMPP.id == user1Id ? user2Id : user1Id
    }
